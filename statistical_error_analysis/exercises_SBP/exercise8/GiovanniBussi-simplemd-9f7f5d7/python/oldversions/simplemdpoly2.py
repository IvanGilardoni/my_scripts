...
@numba.jit(cache=True,fastmath=True)
def _compute_forces(cell, positions, forces,kT, lambd):###, k=30, R0=1.5):###forcecutoff, neighbors, point, forces, k=30, R0=1.5):
    k=30
    R0=1.5
    engconf=0.0
    forces.fill(0.0)
    ...
    for i in range(len(positions)-1):
        j=i+1###for j in range(point[i],point[i+1]):
        ###ja=neighbors[j]
        ...
        e=-0.5*k*R0*R0*np.log(1-distance2/(R0*R0))
        engconf+=e
        fmod=-np.sqrt(distance2)*k/(1-distance2/(R0*R0))-2*kT*lambd/len(positions)**2*np.sqrt(distance2)###2.0*4.0*(6.0*invdistance6*invdistance6-3.0*invdistance6)*invdistance2
        fx=fmod*distancex
        fy=fmod*distancey
        fz=fmod*distancez
        forces[i,0]+=fx
...
#@numba.jit(cache=True,fastmath=True)
def compute_poly(positions):
    nbeads=len(positions)
    b = positions.reshape(positions.shape[0], 1, positions.shape[1])
    distances=np.sqrt(np.einsum('ijk, ijk->ij', positions-b, positions-b))
    Ree=distances[0,nbeads-1]
    Rg2=1/nbeads**2*np.sum(distances**2)
    return Ree,Rg2
...
class SimpleMD:
    ...
    def write_statistics(self,istep,tstep,natoms,engkin,engconf,engint,Ree,Rg2):
         ...
         print("%d %f %f %f %f %f %f %f" %
               (istep,istep*tstep,2.0*engkin/(3.0*natoms),engconf,engkin+engconf,engkin+engconf+engint,Ree,Rg2),
               file=self.write_statistics_fp)
        
###    def write_RR():
###        if self.write_RR is None:
###            self.write_RR=open(self.RRfile,"w")
###            print("%f %f" % (Ree,Rg2),file=self.write_RR)
    ...
    def run(self,parameters):
        ...
        print(positions)
        print(cell)
        engconf= _compute_forces(cell, positions,forces,self.temperature,lambd)
        print(engconf)
        ...
        for istep in range(self.nstep):
            ...
            if (istep+1)%self.nstat==0:
                # compute Ree and Rg2
                Ree,Rg2=compute_poly(positions)

                
                engkin = self.compute_engkin(masses,velocities)
                engconf+=self.temperature*lambd*Rg2
                engint+=self.temperature*lambd*Rg2
                self.write_statistics(istep+1,self.tstep,len(positions),engkin,engconf,engint,Ree,Rg2)
