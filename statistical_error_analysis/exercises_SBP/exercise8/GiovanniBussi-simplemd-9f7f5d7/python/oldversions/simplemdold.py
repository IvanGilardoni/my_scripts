...
def _compute_forces(cell, positions, forcecutoff, neighbors, point, forces):
    ...
    kFENE=30 # k=0 means no FENE
    R0=1.5
    lj=0 # lj=0 means no LJ
    ...
    for i in range(len(positions)):
        for j in range(point[i],point[i+1]):
            ...
            if distance2 <= forcecutoff2:
                ...
                e=(4.0*invdistance6*invdistance6-4.0*invdistance6-engcorrection)*lj
                fmod=(2.0*4.0*(6.0*invdistance6*invdistance6-3.0*invdistance6)*invdistance2)*lj
            engconf+=e
            fx=fmod*distancex
            fy=fmod*distancey
            fz=fmod*distancez
        ...
        for i in range(len(positions)-1):
            ...
            engconf+=e
            fmod=-kFENE/(1-distance2/(R0*R0))###-2*kT*lambd/len(positions)**2*np.sqrt(distance2)###2.0*4.0*(6.0*invdistance6*invdistance6-3.0*invdistance6)*invdistance2
            fx=fmod*distancex
            fy=fmod*distancey
            fz=fmod*distancez
            forces[i,0]+=fx
