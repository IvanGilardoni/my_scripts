...
def _compute_forces(cell, positions, forcecutoff, neighbors, point, forces, lambd, kT):
    ...
    for i in range(len(positions)):
        ...
        for j in range(point[i],point[i+1]):
            ...
            if (ja==i+1):    
                e+=-0.5*kFENE*R0*R0*np.log(1-distance2/(R0*R0))
                fmod+=-kFENE/(1-distance2/(R0*R0))
            ...
            forces[ja,1]-=fy
            forces[ja,2]-=fz
        forces[i,0]+=-2*kT*lambd/(len(positions)**2)*(len(positions)*positions[i,0]-np.sum(positions[:,0]))
        forces[i,1]+=-2*kT*lambd/(len(positions)**2)*(len(positions)*positions[i,1]-np.sum(positions[:,1]))
        forces[i,2]+=-2*kT*lambd/(len(positions)**2)*(len(positions)*positions[i,2]-np.sum(positions[:,2]))
...
def compute_poly(positions):
    ...
    Ree=np.linalg.norm(positions[0,:]-positions[nbeads-1,:])
    Rg2=0
    for i in range(nbeads):
        for j in range(i):
            Rg2+=2*np.linalg.norm(positions[i,:]-positions[j,:])**2      
    Rg2=+1/2*(1/nbeads**2)*Rg2
    return Ree,Rg2
...
class SimpleMD:
    ...
    def write_statistics(self,istep,tstep,natoms,engkin,engconf,engint,Ree,Rg2,lambd):
         ...
         print("%d %f %f %f %f %f %f %f %f" %
               (istep,istep*tstep,2.0*engkin/(3.0*natoms),engconf,engkin+engconf,engkin+engconf+engint,Ree,Rg2,lambd),
    ...
    def run(self,parameters):
        ...
        engconf= _compute_forces(cell, positions, self.forcecutoff, nlist, point, forces, self.temperature, lambd)
        ...
        for istep in range(self.nstep):
            ...
            if (istep>ithreshold):
                lambd+=-(Rg2exp-Rg2)/istep###(istep-ithreshold)
            ...
            if (istep+1)%self.nstat==0:
                ...
                self.write_statistics(istep+1,self.tstep,len(positions),engkin,engconf,engint,Ree,Rg2,lambd)
