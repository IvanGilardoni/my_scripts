    ...
    for i in range(len(positions)):
        for j in range(point[i],point[i+1]):
            ...
            if(longrange):              
                ...
                engconf=engconf-4*(math.pi)*4*natoms**2/(2*3*volume*forcecutoff**3)
    return engconf
...
class SimpleMD:
    ...
    def barostat(self,random,cell,positions,nlist,point,forces,engconf):
            ...
            new_positions[:,0]=positions[:,0]*(new_volume/volume)**(1/3)
            new_positions[:,1]=positions[:,1]*(new_volume/volume)**(1/3)
            new_positions[:,2]=positions[:,2]*(new_volume/volume)**(1/3)
            ...
            new_engconf=_compute_forces(new_cell, new_positions, self.forcecutoff, new_nlist, new_point, forces2, self.longrange)
    ...
    def run(self,parameters):
        ...
        for istep in range(self.nstep):
            ...
            #positions,volume=self.barostat(random,cell,positions,nlist,point,forces,engconf)        
            volume=0
