...
@numba.jit(cache=True,fastmath=True)
def _compute_forces(cell, positions, forcecutoff, neighbors, point, forces, longrange):
    engconf=0.0
    forces.fill(0.0)
    ...
    if(longrange):              
            ...
            engconf=engconf-8*(math.pi)*natoms**2/(3*volume*forcecutoff**3)
    return engconf
...
class SimpleMD:
    ...
    def read_input(self,file):
        with open(file,"r") as f:
            for line in f:
                ...
                key=words[0]
                if key=="longrange":
                    self.longrange=words[1]
                elif key=="temperature":
                    self.temperature=float(words[1])
                elif key=="pressure":
                    self.pressure=float(words[1])
                elif key=="delta":
                    self.delta=float(words[1])
                elif key=="tstep":
    ...
    def barostat(self,random,cell,positions,nlist,point,forces,engconf):
            natoms=len(positions)
            volume=cell[0]*cell[1]*cell[2]
            
            print(positions[0,0])
            ## delta=0.1
            #new_volume=volume+(random.Gaussian(shape=1))*self.delta#(2*np.random.rand()-1)*self.delta
            new_volume=volume+(2*np.random.rand()-1)*self.delta
            #new_volume=volume
            #print('energy',engconf)
            
            # is cell a square?
            new_cell=np.zeros(3)
            new_cell[0]=new_volume**(1/3)
            new_cell[1]=new_volume**(1/3)
            new_cell[2]=new_volume**(1/3)
            
            new_positions=np.zeros((natoms,3))
            new_positions=positions*(new_volume/volume)**(1/3)
            #print(new_cell)
            forces2=+forces
            
            #check_list=self.check_list(new_positions,positions,self.listcutoff,self.forcecutoff)
            
            #new_nlist=+nlist
            #new_point=+point
            
            #if check_list:
            #    _compute_list(new_cell, new_positions, self.listcutoff, new_nlist, new_point)
                ###positions=+new_positions
            
            new_engconf=_compute_forces(new_cell, new_positions, self.forcecutoff, nlist, point, forces2,self.longrange)
            
            
            print('energy post',new_engconf)

            alpha=np.exp(natoms*np.log(new_volume/volume))*np.exp(-1/self.temperature*(new_engconf-engconf+self.pressure*(new_volume-volume)))
            if alpha>1:
                alpha=1
            
            if np.random.rand()<alpha:            
                positions[:,0]=+new_positions[:,0]
                positions[:,1]=+new_positions[:,1]
                positions[:,2]=+new_positions[:,2]
            ...
            return positions,volume,forces,cell,engconf
    ...
    def write_statistics(self,istep,tstep,natoms,engkin,engconf,engint,volume):
         ...
         print("%d %f %f %f %f %f %f" %
               (istep,istep*tstep,2.0*engkin/(3.0*natoms),engconf,engkin+engconf,engkin+engconf+engint,volume),
    ...
    def run(self,parameters):
        ...
        engconf= _compute_forces(cell, positions, self.forcecutoff, nlist, point, forces, self.longrange)
        ...
        for istep in range(self.nstep):
            ...
            positions,volume,forces,cell,engconf=self.barostat(random,cell,positions,nlist,point,forces,engconf)
            ###
            ...
            if (istep+1)%self.nstat==0:
                ...
                self.write_statistics(istep+1,self.tstep,len(positions),engkin,engconf,engint,volume)
