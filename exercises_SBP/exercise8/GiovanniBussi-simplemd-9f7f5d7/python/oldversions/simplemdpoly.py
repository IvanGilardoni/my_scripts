...
@numba.jit(cache=True,fastmath=True)
def _compute_forces(cell, positions, forcecutoff,neighbors,point,forces,kT, lambd):###, k=30, R0=1.5):
    k=30
    R0=1.5
    engconf=0.0
    forces.fill(0.0)
    ...
    for i in range(len(positions)):
        for j in range(point[i],point[i+1]):
            ...
            if distance2 <= forcecutoff2:
                ...
                if j==i+1:
                    ## only to try without LJ
                    ##e=0
                    ##fmod=0
                    ###
                    
                    e+=-0.5*k*R0*R0*np.log(1-distance2/(R0*R0))
                    fmod+=-k/(1-distance2/(R0*R0))-2*kT*lambd/len(positions)**2*np.sqrt(distance2)
                engconf+=e
                fx=fmod*distancex
                fy=fmod*distancey
                fz=fmod*distancez
