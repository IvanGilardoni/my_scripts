...
@numba.jit(cache=True,fastmath=True)
def _compute_forces(cell, positions, forcecutoff, neighbors, point, forces, kT, lambd):
    k=30
    R0=1.5
    engconf=0.0
    forces.fill(0.0)
        ...
        for j in range(point[i],point[i+1]):
            ...
            if distance2 <= forcecutoff2:
                ...
                if j==i+1:
                    ...
                    fmod+=-np.sqrt(distance2)*k/(1-distance2/(R0*R0))-2*kT*lambd/len(positions)**2*np.sqrt(distance2)
